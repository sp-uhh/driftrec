...
from .predictors import PredictorRegistry, ReverseDiffusionPredictor
...
def get_pc_sampler(
    ...
    **kwargs
    ...
    N = N if N is not None else sde.N
    timesteps = torch.linspace(sde.T, eps, N, device=y.device)
    ones = torch.ones(N, y.shape[0], device=y.device)
    vec_ts = timesteps[:, None] * ones

    def pc_sampler():
        """The PC sampler function."""
        with torch.no_grad():
            xt = sde.prior_sampling(y).to(y.device)
            iterator = tqdm.trange(N, unit="sampling step") if use_tqdm else range(N)
            for i in iterator:
                vec_t = vec_ts[i]
                xt, xt_mean = corrector.update_fn(xt, vec_t, y)
                xt, xt_mean = predictor.update_fn(xt, vec_t, y)
            x_result = xt_mean if denoise else xt
            ns = N * (corrector.n_steps + 1)
            return x_result, ns
...
def get_ode_sampler(
    ...
    denoise=True, rtol=1e-3, atol=1e-3,
    ...
    def ode_sampler(z=None, **kwargs):
        ...
        with torch.no_grad():
            ...
            x = sde.prior_sampling(y).to(device)

            def ode_func(t, x):
                x = from_flattened_numpy(x, y.shape).to(device).type(torch.float32)
            ...
            nfe = solution.nfev
            x = torch.tensor(solution.y[:, -1]).reshape(y.shape).to(device).type(torch.float32)
