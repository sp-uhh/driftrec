...
from .ncsnpp_utils.up_or_down_sampling import pad_to_fit_2d
...
class NCSNpp(nn.Module):
    ...
    def add_argparse_args(parser):
        parser.add_argument("--ch_mult", type=int, nargs="+", default=[1, 1, 2, 2, 2, 2, 2],
                            help="Channel multiplier per resolution")
        parser.add_argument("--num_res_blocks", type=int, default=2,
                            help="Number of residual blocks per resolution")
        parser.add_argument("--nf", type=int, default=128,
                            help="Time embedding dimensionality")
        return parser
    ...
    def __init__(self,
        ...
        **unused_kwargs
        ...
        self.output_layer = nn.Conv2d(self.in_channels, self.out_channels, 1)
        ...
        if self.conditional:
            ...
            logger.info("Running NCSN++ unconditionally")
            temb_dim = None
    ...
    def forward(self, x, time, sigmas):
        # timestep/noise_level embedding; only for continuous training
        modules = self.all_modules
        m_idx = 0

        if self.conditional:
            if self.embedding_type == 'fourier' and self.conditional:
                # Gaussian Fourier features embeddings.
        ...
        for i_level in range(self.num_resolutions):
            ...
            for i_block in range(self.num_res_blocks):
                ...
                if i_level in self._attn_levels:
                    h = modules[m_idx](h)
                    m_idx += 1
                hs.append(h)
        ...
        for i_level in reversed(range(self.num_resolutions)):
            ...
            if self.progressive != 'none':
                if i_level == self.num_resolutions - 1:
                    ...
                    if self.progressive == 'output_skip':
                        ...
                        pyramid = pad_to_fit_2d(pyramid, pyramid_h)  # pad pyramid to fit pyramid_h for dims -1 and -2
                        m_idx += 1
            ...
            if i_level != 0:
                if self.resblock_type == 'ddpm':
                    ...
                    m_idx += 1
                # pad h so that it fits hs[-1] for dims -1 and -2
                h = pad_to_fit_2d(h, hs[-1])
